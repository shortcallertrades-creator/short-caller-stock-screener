...
console.log('Initializing update_charts.js...');
...
console.log(`Posted file path: ${postedFile}`);
console.log(`Ticker data file path: ${tickerDataFile}`);

function loadPosted() {
  try {
    console.log('Loading posted tickers from file...');
    const data = fs.readFileSync(postedFile, 'utf8');
    const posted = new Set(JSON.parse(data));
    console.log(`Loaded ${posted.size} previously posted tickers`);
    return posted;
  } catch (error) {
    console.log('No posted tickers file found, starting fresh');
    return new Set();
  }
}
...
function parsePrice(html) {
  try {
    const priceRegex = /<span[^>]*class="[^"]*quote-price[^"]*"[^>]*>([^<]+)<\/span>/i;

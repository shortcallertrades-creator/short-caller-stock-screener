...
const { postNewTickersToTikTok } = require('./tiktok_poster');
  ...
  try {
    ...
    if (newTickers.length > 0 || Object.keys(tickerData).length > 0) {
      ...
      if (newTickers.length > 0) {
        ...
        // Post new tickers to TikTok
        log('\n📱 Starting TikTok posting...');
        const updatedTickerData = loadTickerData();
        await postNewTickersToTikTok(newTickers, updatedTickerData);
      }
    } else {
